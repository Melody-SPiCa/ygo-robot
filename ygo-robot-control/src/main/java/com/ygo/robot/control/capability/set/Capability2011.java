...
@Slf4j
@Service
public class Capability2011 extends RobotCapability {
  ...
  public CallbackMsg postProcess() {
    ...
    try {
      ...
      val reply = String.format("卡片详情：%s\n卡片裁定：%s", cardUrl, Constant.YGO_RULE_HOST + text);
      return CallbackMsg.ofGroup(groupId, MsgUtils.builder().text(reply).build());
    } catch (Exception e) {
      log.error("【Capability-2011】卡片查询失败");
      return CallbackMsg.ofGroup(groupId, MsgUtils.builder().text("小蓝查不到...").build());
    } finally {
      isInSearch = false;
    }
  ...
  public String matchesRegex() {
    //    return "^(小蓝查调整|小蓝查裁定|小蓝查卡调整|小蓝查卡详情)(.{1,64})$";
    return "^小蓝查(?:调整|裁定|卡调整|卡详情)(?!.*(?:调整|图|裁定|详情))(.{1,64})$";
  }
  ...
  private static class Constant {
    ...
    /** 卡片裁定查询服务器 */
    private static final String YGO_RULE_HOST =
        "https://ocg-rule.readthedocs.io/zh-cn/latest/search.html?q=";
  }
